...
const params = {
  shatter: 0.0,    // 0 = Intact, 1 = Exploded
  rotation: 0.0,   // Spin the fragments
  subdivisions: 5  // Grid detail
};
...
const sketch = ({ width, height }) => {
  ...
  pane.addInput(params, 'shatter', { min: 0, max: 2.0 });
  pane.addInput(params, 'rotation', { min: -5, max: 5 });

  // Helper to calculate the center of a triangle
  ...
  return ({ context, width, height, time }) => {
    ...
    fragments.forEach(frag => {
      // Calculate blast vector (away from logo center)
      ...
      context.save();
      // Apply the explosion movement
      context.translate(offsetX, offsetY);
      
      // Apply a spin based on rotation param
      context.translate(frag.center.x, frag.center.y);
      context.rotate(params.shatter * params.rotation);
      context.translate(-frag.center.x, -frag.center.y);
